...
def _get_ome_version(group) -> str | None:
    attrs = dict(group.attrs)
    if ome_attr := attrs.get("ome"):
        return ome_attr["version"]
    elif "multiscales" in attrs:
        return attrs["multiscales"][0]["version"]
    elif "plate" in attrs:
        return attrs["plate"]["version"]
    elif "bioformats2raw.layout" in attrs:
        if "0" in group:
            return _get_ome_version(group["0"])
    raise NotImplementedError(f"Unable to determine OME-Zarr version for {group}")
...
if __name__ == "__main__":
    ...
    for url in URLS:
        ...
        ome_version = _get_ome_version(group)
        if ome_version == "0.3":
            print(url)
            continue
        # records.append(
        #     {"url": url, "zarr_format": zarr_format, "ome_version": ome_version}
        # )

        yg = yaozarrs.open_group(url)
        md = yg.ome_metadata()
        print(md)
