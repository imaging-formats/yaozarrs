"""Tests for OME-Zarr v0.6 write functionality."""
...
from yaozarrs.write.v06 import (
    Bf2RawBuilder,
    LabelsBuilder,
    PlateBuilder,
    _write,
...
def _make_image(name: str, dims_scale: Mapping[str, float]) -> v06.Image:
    """Create a simple v06.Image model for testing."""
    dims = [DimSpec(name=n, scale=s) for n, s in dims_scale.items()]
    return v06.Image(multiscales=[v06.Multiscale.from_dims(dims, name=name)])


def _make_multiscale_image(
    name: str = "test", n_levels: int = 3
) -> tuple[list[np.ndarray], v06.Image]:
    ...
    ms = v06.Multiscale.from_dims(dims, name=name, n_levels=n_levels)
    return data, v06.Image(multiscales=[ms])


def _make_plate(
    n_rows: int = 2, n_cols: int = 2
) -> tuple[v06.Plate, dict[tuple[str, str, str], tuple[v06.Image, list[np.ndarray]]]]:
    """Create a simple plate for testing."""
    row_names = [chr(ord("A") + i) for i in range(n_rows)]
    col_names = [f"{i + 1:02d}" for i in range(n_cols)]

    wells = [
        v06.PlateWell(path=f"{r}/{c}", rowIndex=ri, columnIndex=ci)
        for ri, r in enumerate(row_names)
        for ci, c in enumerate(col_names)
    ]
    plate = v06.Plate(
        plate=v06.PlateDef(
            columns=[v06.Column(name=name) for name in col_names],
            rows=[v06.Row(name=name) for name in row_names],
            wells=wells,
        )
    )
    images = {
    ...
    image = v06.Image(
        multiscales=[
            v06.Multiscale.from_dims(
                [DimSpec(name="c"), DimSpec(name="y"), DimSpec(name="x")],
                n_levels=2,
            )
        ]
    )
    ...
    image = _make_image("rgb_image", {"c": 1.0, "y": 0.5, "x": 0.5}).model_copy(
        update={
            "omero": v06.Omero(
                channels=[
                    v06.OmeroChannel(
                        window=v06.OmeroWindow(start=0, min=0, end=255, max=255),
                        color="FF0000",
                        label="Red",
                        active=True,
                    ),
                ...
                rdefs=v06.OmeroRenderingDefs(model="color"),
            )
        }
    )
    write_image(
...
def test_write_plate_multi_field(tmp_path: Path, writer: ZarrWriter) -> None:
    ...
    plate = v06.Plate(
        plate=v06.PlateDef(
            columns=[v06.Column(name="01")],
            rows=[v06.Row(name="A")],
            wells=[v06.PlateWell(path="A/01", rowIndex=0, columnIndex=0)],
        )
    )
...
def test_plate_builder_immediate_write(tmp_path: Path, writer: ZarrWriter) -> None:
    ...
    wells_data: dict[tuple[str, str], dict[str, tuple[v06.Image, list]]] = {}
...
def test_plate_builder_multiscale_error(tmp_path: Path) -> None:
    ...
    image = v06.Image(
        multiscales=[
            v06.Multiscale.from_dims(
                dims, name="multiscale_0", coordinate_system="cs0"
            ),
            v06.Multiscale.from_dims(
                dims, name="multiscale_1", coordinate_system="cs1"
            ),
        ]
    )
...
def test_extra_attributes_roundtrip(tmp_path: Path, writer: ZarrWriter) -> None:
    ...
    from yaozarrs.v06 import OMEZarrGroupJSON
